...
> [!NOTE]
> Questo esempio utilizza il trasporto legacy HTTP+SSE e si rivolge a un SDK compatibile
> con MCP `2025-11-25`. I nuovi server remoti dovrebbero utilizzare il supporto Streamable
> HTTP `2026-07-28`.

Questo servizio fornisce operazioni di calcolatrice di base tramite il Model Context Protocol (MCP) usando Spring Boot con trasporto WebFlux. È progettato come un esempio semplice per principianti che imparano sulle implementazioni MCP.

Per maggiori informazioni, consulta la documentazione di riferimento del [MCP Server Boot Starter](https://docs.spring.io/spring-ai/reference/api/mcp/mcp-server-boot-starter-docs.html).

## Panoramica

Il servizio presenta:
- Supporto per SSE (Server-Sent Events)
- Registrazione automatica degli strumenti usando l'annotazione `@Tool` di Spring AI
- Funzioni base della calcolatrice:
  - Addizione, sottrazione, moltiplicazione, divisione
  - Calcolo della potenza e della radice quadrata
  - Modulo (resto) e valore assoluto
  - Funzione di aiuto per la descrizione delle operazioni

## Caratteristiche

Questo servizio di calcolatrice offre le seguenti funzionalità:
...
2. **Operazioni Avanzate**:
   - Calcolo della potenza (elevamento a esponente)
...
Il servizio espone i seguenti endpoint API attraverso il protocollo MCP:
...
- `squareRoot(number)`: Calcola la radice quadrata (con controllo numero negativo)
- `modulus(a, b)`: Calcola il resto della divisione
- `absolute(number)`: Calcola il valore assoluto
- `help()`: Ottieni informazioni sulle operazioni disponibili
...
Il progetto include un esempio di client LangChain4j in `com.microsoft.mcp.sample.client.LangChain4jClient` che dimostra come integrare il servizio calcolatrice con LangChain4j e modelli GitHub:

### Prerequisiti

1. **Configurazione Token GitHub**:
   ...
   b. Clicca "Generate new token" → "Generate new token (classic)"
   
   c. Dai un nome descrittivo al token
   
   d. Seleziona i seguenti ambiti:
      - `repo` (Controllo completo sui repository privati)
      - `read:org` (Lettura membri org e team, lettura progetti org)
      - `gist` (Creazione di gist)
      - `user:email` (Accesso agli indirizzi email utente (sola lettura))
   
   e. Clicca "Generate token" e copia il nuovo token
   ...
   g. Per un setup persistente, aggiungilo alle variabili d'ambiente tramite le impostazioni di sistema
...
- Uso di LangChain4j per creare un bot chat che sfrutta le operazioni della calcolatrice
- Integrazione con i modelli AI GitHub (ora usando modello phi-4)
...
3. Ottenere informazioni di aiuto sulle operazioni disponibili
...
1. **Installa ed esegui MCP Inspector** in una nuova finestra terminale:
   ```bash
...
4. **Usa gli strumenti**:
   - Clicca "List Tools" per vedere le operazioni calcolatrice disponibili
   - Seleziona uno strumento e clicca "Run Tool" per eseguire un'operazione

![Screenshot di MCP Inspector](../../../../../../translated_images/it/tool.c75a0b2380efcf1a.webp)
...
- Avviare il servizio calcolatrice MCP all'interno del contenitore

Puoi accedere al servizio su `http://localhost:8080` una volta che il contenitore è in esecuzione.

## Risoluzione dei Problemi

### Problemi Comuni con il Token GitHub

1. **Problemi di Permessi con il Token**: Se ricevi un errore 403 Forbidden, verifica che il token abbia i permessi corretti come indicato nei prerequisiti.

2. **Token Non Trovato**: Se ricevi un errore "No API key found", assicurati che la variabile d'ambiente GITHUB_TOKEN sia correttamente impostata.

3. **Limitazioni di Velocità**: L'API di GitHub ha limiti di velocità. Se incontri un errore di limite (codice 429), attendi qualche minuto prima di riprovare.

4. **Scadenza del Token**: I token GitHub possono scadere. Se dopo un po' ricevi errori di autenticazione, genera un nuovo token e aggiorna la variabile d'ambiente.
