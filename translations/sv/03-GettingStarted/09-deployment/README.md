...
> [!NOTE]
> Konfigurationsexempel som använder en `/sse`-endpoint riktar sig mot den äldre HTTP+SSE-
> transporten. MCP `2026-07-28` fjärrservrar använder Streamable HTTP, normalt vid en
> serverdefinierad endpoint som `/mcp`.
...
Om din server är tänkt att användas genom att köras på användarnas maskiner, kan du följa följande steg:

1. **Ladda ner servern**. Om du inte skrev servern, ladda då först ner den till din maskin.
1. **Starta serverprocessen**: Kör din MCP-serverapplikation

För SSE (behövs ej för stdio-typ server)
...
1. För att prova lokalt, skapa en *mcp.json*-fil i en *.vscode*-katalog och lägg till följande innehåll:
  ...
  När SSE-servern är startad kan du klicka på play-ikonen i JSON-filen, du bör nu se verktyg på servern bli upptagna av GitHub Copilot, se verktygsikonen.
...
Där har du det, distribuera lokalt eller till Azure genom dessa steg.
