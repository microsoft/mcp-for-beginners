...
- Entender lo que un cliente puede hacer.
- Escribir tu propio cliente.
- Conectar y probar el cliente con un servidor MCP para asegurar que funcione como se espera.

## ¿Qué implica escribir un cliente?

Para escribir un cliente, necesitarás hacer lo siguiente:

- **Importar las bibliotecas correctas**. Usarás la misma biblioteca que antes, solo construcciones diferentes.
- **Instanciar un cliente**. Esto implica crear una instancia de cliente y conectarla al método de transporte elegido.
- **Decidir qué recursos listar**. Tu servidor MCP incluye recursos, herramientas y solicitudes, necesitas decidir cuáles listar.
- **Integrar el cliente en una aplicación host**. Una vez que conoces las capacidades del servidor, necesitas integrar esto en tu aplicación host para que si un usuario escribe una solicitud u otro comando se invoque la función correspondiente del servidor.
...
- Creamos una instancia de cliente y lo conectamos usando stdio para transporte.
- Listamos solicitudes, recursos y herramientas y las invocamos todas.

Ahí lo tienes, un cliente que puede comunicarse con un Servidor MCP.

Tomémonos nuestro tiempo en la próxima sección de ejercicios para desglosar cada fragmento de código y explicar qué está pasando.

## Ejercicio: Escribiendo un cliente

Como se dijo arriba, tomémonos nuestro tiempo explicando el código, y por supuesto codifica junto si quieres.

### -1- Importar las bibliotecas

Importemos las bibliotecas que necesitamos, necesitaremos referencias a un cliente y a nuestro protocolo de transporte elegido, stdio. stdio es un protocolo para cosas destinadas a ejecutarse en tu máquina local. SSE es otro protocolo de transporte que mostraremos en futuros capítulos pero es tu otra opción. Por ahora, sigamos con stdio.
...
Desde ahí, puedes importar las bibliotecas necesarias en tu código cliente.
...
Necesitaremos crear una instancia del transporte y de nuestro cliente:
...
- Creado una instancia de transporte stdio. Nota cómo especifica el comando y los argumentos para encontrar y arrancar el servidor, ya que es algo que necesitaremos hacer al crear el cliente.
...
- Instanciado un objeto de parámetros del servidor que usaremos para ejecutar el servidor y así conectar nuestro cliente.
- Definido un método `run` que a su vez llama a `stdio_client` que inicia una sesión cliente.
...
- Importado las bibliotecas necesarias.
- Creado un transporte stdio y creado un cliente `mcpClient`. Este último es algo que usaremos para listar e invocar funciones en el Servidor MCP.
...
- Creado un método principal que configura un transporte SSE apuntando a `http://localhost:8080` donde nuestro servidor MCP estará ejecutándose.
- Creado una clase cliente que toma el transporte como parámetro del constructor.
- En el método `run`, creamos un cliente MCP síncrono usando el transporte e inicializamos la conexión.
- Usado transporte SSE (Server-Sent Events) adecuado para comunicación HTTP con servidores MCP en Java Spring Boot.

#### Rust

Nota que este cliente Rust asume que el servidor es un proyecto hermano llamado "calculator-server" en el mismo directorio. El código a continuación iniciará el servidor y se conectará a él.
...
Ahora, tenemos un cliente que puede conectarse si el programa se ejecuta. Sin embargo, no lista realmente sus funciones, así que hagámoslo a continuación:
...
Arriba hay un ejemplo de cómo podemos listar las herramientas en el servidor. Para cada herramienta, luego imprimimos su nombre.
...
- Usado `ping()` para verificar que la conexión al servidor funciona.
...
Genial, ahora hemos capturado todas las funciones. Ahora la pregunta es ¿cuándo las usamos? Bueno, este cliente es bastante simple, simple en el sentido de que necesitaremos llamar explícitamente a las funciones cuando las queramos. En el próximo capítulo, crearemos un cliente más avanzado que tendrá acceso a su propio modelo de lenguaje grande, LLM. Por ahora, veamos cómo podemos invocar las funciones en el servidor:

#### Rust

En la función principal, después de inicializar el cliente, podemos inicializar el servidor y listar algunas de sus funciones.
...
- Leemos un recurso, lo llamamos usando `readResource()` especificando `uri`. Esto es lo que probablemente se ve en el servidor:
...
- Obtenemos prompt, para obtener un prompt, llamamos `getPrompt()` con `name` y `arguments`. El código del servidor es así:
...
Añade la siguiente entrada a la sección "scripts" en *package.json*:
...
Primero, asegúrate de que tu servidor MCP esté funcionando en `http://localhost:8080`. Luego ejecuta el cliente:
...
En esta asignación, usarás lo aprendido para crear un cliente pero crear uno propio.

Aquí hay un servidor que puedes usar para llamar a través de tu código cliente, intenta añadir más funciones al servidor para hacerlo más interesante.
...
Consulta este proyecto para ver cómo puedes [añadir solicitudes y recursos](https://github.com/modelcontextprotocol/csharp-sdk/blob/main/samples/EverythingServer/Program.cs).

También, revisa este enlace para saber cómo invocar [solicitudes y recursos](https://github.com/modelcontextprotocol/csharp-sdk/blob/main/src/ModelContextProtocol/Client/).

### Rust

En la [sección anterior](../../../../03-GettingStarted/01-first-server), aprendiste cómo crear un servidor MCP simple con Rust. Puedes seguir construyendo sobre eso o revisar este enlace para más ejemplos de servidores MCP basados en Rust: [Ejemplos de Servidor MCP](https://github.com/modelcontextprotocol/rust-sdk/tree/main/examples/servers)

## Solución

La **carpeta de solución** contiene implementaciones completas y listas para ejecutarse de clientes que demuestran todos los conceptos cubiertos en este tutorial. Cada solución incluye tanto código cliente como servidor organizado en proyectos separados y autónomos.
...
- **Estructura del proyecto funcional** con dependencias y configuración adecuadas
- **Scripts para construir y ejecutar** para configuración y ejecución sencilla
...
| **TypeScript** | [`client_example_typescript.ts`](../../../../03-GettingStarted/02-client/client_example_typescript.ts) | Cliente TypeScript completo con soporte completo del protocolo MCP |
...
| **Rust** | [`client_example_rust.rs`](../../../../03-GettingStarted/02-client/client_example_rust.rs) | Cliente Rust completo usando Tokio para operaciones asincrónicas |

Cada ejemplo completo incluye:

- ✅ **Establecimiento de conexión** y manejo de errores
- ✅ **Descubrimiento del servidor** (herramientas, recursos, solicitudes donde aplique)
- ✅ **Operaciones de calculadora** (suma, resta, multiplicación, división, ayuda)
...
| **Carpeta Solución** | **Ejemplos Completos** |
|--------------------|--------------------- |
...
Ambos enfoques son valiosos: usa la **carpeta solución** para proyectos completos y los **ejemplos completos** para aprendizaje y referencia.

## Conclusiones clave

Las conclusiones clave de este capítulo sobre clientes son las siguientes:

- Pueden usarse tanto para descubrir como para invocar funcionalidades en el servidor.
- Pueden iniciar un servidor mientras él mismo inicia (como en este capítulo), pero los clientes también pueden conectarse a servidores en ejecución.
- Son una excelente manera de probar las capacidades del servidor junto a alternativas como el Inspector, tal y como se describió en el capítulo anterior.
