...
> [!NOTE]
> Los ejemplos de cliente en Java se conectan a través del transporte HTTP+SSE heredado y
> apuntan a las APIs del SDK MCP `2025-11-25`. Use un SDK compatible con `2026-07-28` y
> HTTP Streamable para nuevos clientes remotos.

Hasta ahora, has visto cómo crear un servidor y un cliente. El cliente ha podido llamar al servidor explícitamente para listar sus herramientas, recursos y prompts. Sin embargo, este no es un enfoque muy práctico. Tus usuarios viven en la era agentiva y esperan usar prompts y comunicarse con un LLM en su lugar. No les importa si usas MCP para almacenar tus capacidades; simplemente esperan interactuar usando lenguaje natural. ¿Entonces, cómo solucionamos esto? La solución es añadir un LLM al cliente.

## Visión general

En esta lección nos enfocamos en añadir un LLM a tu cliente y mostramos cómo esto proporciona una experiencia mucho mejor para tu usuario.
...
- Proporcionar una mejor experiencia para el usuario final en el lado cliente.

## Enfoque

Intentemos entender el enfoque que necesitamos tomar. Añadir un LLM parece simple, pero ¿realmente lo haremos?
...
1. Añadir un LLM y pasar las capacidades guardadas y sus esquemas en un formato que el LLM entienda.

1. Manejar un prompt del usuario pasándoselo al LLM junto con las herramientas listadas por el cliente.

Genial, ahora que entendemos cómo podemos hacer esto a alto nivel, probémoslo en el ejercicio a continuación.
...
- Selecciona Tokens de acceso personal – Haz clic en Tokens con permisos específicos y luego en Generar nuevo token.
...
Primero, necesitarás añadir las dependencias de LangChain4j a tu archivo `pom.xml`. Añade estas dependencias para habilitar la integración MCP y la API MiniMax compatible con OpenAI:
...
`OPENAI_BASE_URL` no está configurado, `MINIMAX_REGION` soporta `global_en` y `cn_zh`.
...
- **Añadido dependencias de LangChain4j**: Requeridas para integración MCP y API MiniMax compatible con OpenAI
- **Importado las librerías de LangChain4j**: Para la integración MCP y funcionalidad de modelo de chat OpenAI
...
- **Creado un cliente MCP**: Que gestionará la comunicación con el servidor
- **Usado soporte MCP incorporado de LangChain4j**: Lo que simplifica la integración entre LLMs y servidores MCP
...
Una vez tengas tu servidor MCP en Rust, abre una terminal y navega al mismo directorio que el servidor. Luego ejecuta el siguiente comando para crear un nuevo proyecto de cliente LLM:
...
> [!NOTE]
> No hay una biblioteca oficial de Rust para OpenAI, sin embargo, el crate `async-openai` es una [biblioteca mantenida por la comunidad](https://platform.openai.com/docs/libraries/rust#rust) que es comúnmente usada.
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // TODO: Obtener lista de herramientas MCP
...
Este código configura una aplicación básica en Rust que se conectará a un servidor MCP y GitHub Models para interacciones LLM.

> [!IMPORTANT]
> Asegúrate de configurar la variable de entorno `OPENAI_API_KEY` con tu token de GitHub antes de ejecutar la aplicación.

Genial, para nuestro siguiente paso, listemos las capacidades del servidor.
...
- Creado un método `run` responsable de manejar el flujo de nuestra aplicación. Hasta ahora solo lista las herramientas, pero añadiremos más pronto.
...
- Listar recursos y herramientas e imprimirlos. Para las herramientas también listamos `inputSchema` que usaremos después.
...
- Enumerado las herramientas disponibles en el servidor MCP
- Para cada herramienta, listado nombre, descripción y su esquema. Este último es algo que utilizaremos para llamar a las herramientas en breve.
...
// - Listar herramientas disponibles desde el servidor MCP
...
Recuperar herramientas del servidor MCP se realiza usando el método `list_tools`. En tu función `main`, después de configurar el cliente MCP, añade el siguiente código:
...
2. A continuación, actualicemos nuestro código cliente para aprovechar esta función así:
    ...
    Aquí, añadimos una llamada a `convert_to_llm_tool` para convertir la respuesta de herramienta MCP a algo que podamos alimentar al LLM más adelante.
...
- Definida funcionalidad que crea una FunctionDefinition que se pasa a un ChatCompletionsDefinition. Este último es algo que el LLM puede entender.

2. Veamos cómo podemos actualizar algún código existente para aprovechar esta función anterior:
...
- El framework maneja automáticamente la conversión de esquemas de herramientas y llamadas a funciones tras bambalinas
...
Para convertir la respuesta de herramienta MCP a un formato que el LLM pueda entender, añadiremos una función auxiliar que formatee la lista de herramientas. Añade el siguiente código a tu archivo `main.rs` debajo de la función `main`. Esto se llamará al hacer solicitudes al LLM:
...
Genial, ya estamos preparados para manejar solicitudes de usuario, así que abordemos eso a continuación.

### -4- Manejar solicitud de usuario
...
1. Añade un método que se usará para llamar a nuestro LLM:
    ...
    - Añadido un método `callTools`.
    - El método toma una respuesta del LLM y verifica qué herramientas se han llamado, si alguna:
...
Genial, aquí listado el código completo:
...
2. Ahora, añadamos la función que llamará al LLM:
    ...
    - Después, inspeccionamos el resultado para ver qué funciones debemos llamar, si es que hay alguna.
...
3. Paso final, actualicemos nuestro código principal:
    ...
    # preguntar a LLM qué herramientas usar, si las hay
    ...
    - Llamando a una herramienta MCP vía `call_tool` usando una función que el LLM pensó que debíamos llamar basada en nuestro prompt.
    - Imprimiendo el resultado de la llamada a la herramienta al servidor MCP.
...
2. Un último paso, veamos si el LLM piensa que debemos llamar a una función:
    ...
    - Para cada llamada a herramienta, parsear el nombre y argumentos y llamar a la herramienta en el servidor MCP usando el cliente MCP. Finalmente imprimimos los resultados.
...
```java
try {
    // Ejecutar solicitudes en lenguaje natural que utilizan automáticamente herramientas MCP
...
Aquí es donde ocurre la mayor parte del trabajo. Llamaremos al LLM con el mensaje inicial del usuario, luego procesaremos la respuesta para ver si es necesario llamar a alguna herramienta. Si es así, llamaremos a esas herramientas y continuaremos la conversación con el LLM hasta que no se necesiten más llamadas a herramientas y tengamos una respuesta final.
...
Esta función toma el cliente LLM, una lista de mensajes (incluyendo el mensaje del usuario), las herramientas del servidor MCP y envía una solicitud al LLM, devolviendo la respuesta.

La respuesta del LLM contendrá un arreglo de `choices`. Necesitaremos procesar el resultado para ver si hay `tool_calls` presentes. Esto nos indica que el LLM está solicitando que se llame a una herramienta específica con argumentos. Agrega el siguiente código al final de tu archivo `main.rs` para definir una función que maneje la respuesta del LLM:
...
Si hay `tool_calls`, extrae la información de la herramienta, llama al servidor MCP con la solicitud de la herramienta y añade los resultados a los mensajes de la conversación. Luego continúa la conversación con el LLM y los mensajes se actualizan con la respuesta del asistente y los resultados de la llamada a la herramienta.

Para extraer la información de las llamadas a herramientas que el LLM devuelve para las llamadas al MCP, añadiremos otra función auxiliar para extraer todo lo necesario para realizar la llamada. Agrega el siguiente código al final de tu archivo `main.rs`:
...
Toma el código del ejercicio y desarrolla el servidor con algunas herramientas más. Luego crea un cliente con un LLM, como en el ejercicio, y pruébalo con diferentes mensajes para asegurarte de que todas las herramientas de tu servidor se llaman de manera dinámica. Esta forma de construir un cliente significa que el usuario final tendrá una excelente experiencia, ya que puede usar mensajes en lugar de comandos exactos del cliente y ser ajeno a cualquier llamada al servidor MCP.
