...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-19T09:01:02+00:00",
  "source_file": "02-Security/mcp-security-best-practices-2025.md",
  "language_code": "mo"
}
-->
# MCP 安全最佳實踐 - 2025年12月更新
...
**受保護的令牌儲存：**  
   - 使用 Azure Key Vault 或類似安全憑證庫儲存所有秘密  
   - 實施令牌靜態與傳輸加密  
   - 定期輪替憑證並監控未授權存取  
...
**安全會話實踐：**  
   ...
   - **用戶綁定**：使用 `<user_id>:<session_id>` 格式將會話 ID 綁定用戶身份，防止跨用戶會話濫用  
...
**工具中毒防範：**  
   - **工具元資料驗證**：實施工具定義完整性檢查，監控異常變更  
   - **動態工具監控**：監控執行時行為，設置異常執行模式警報  
...
**內容安全實作：**  
   ...
   - **行為分析**：實施執行時行為監控，偵測 MCP 伺服器及工具異常  
...
**快速回應能力：**  
   - **自動化回應**：實施自動化憑證輪替與威脅遏制系統  
   - **回滾程序**：能快速回復至已知良好配置與元件  
   - **鑑識能力**：詳細審計軌跡與日誌以供事件調查  
...
**執行時應用防護：**  
   - **執行時應用自我防護 (RASP)**：部署 RASP 技術進行即時威脅偵測  
