...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-12T16:33:01+00:00",
...
本課程設計時充分考慮您的學習歷程，我們將從您已熟悉的簡單概念開始，並透過您喜愛的程式語言逐步建立專業技能。每個步驟都包含清晰的解說、實用範例，以及豐富的鼓勵。

完成這趟旅程後，您將有信心打造自己的 MCP 伺服器，將它們整合到熱門 AI 平台，並理解這項技術如何重塑 AI 開發的未來。讓我們一起開始這段令人振奮的冒險吧！

### 官方文件與規範

這些資源隨著您的理解加深而更有價值，但不必急著全部閱讀。先從您最感興趣的部分開始！
...
- 🧑‍💻 [MCP GitHub 倉庫](https://github.com/modelcontextprotocol) – 這裡有多種程式語言的 SDK、工具和程式碼範例。就像實用範例和現成元件的寶庫。
- 🌐 [MCP 社群](https://github.com/orgs/modelcontextprotocol/discussions) – 加入其他學習者和經驗豐富的開發者，一起討論 MCP。這是一個支持性的社群，歡迎提問並自由分享知識。
  
## 學習目標

完成本課程後，您將感到自信且充滿熱忱。以下是您將達成的目標：

• **理解 MCP 基礎**：您將掌握模型上下文協議是什麼，以及為何它正在革新 AI 應用程式的協作方式，並透過類比和範例讓概念更易理解。

• **建立您的第一個 MCP 伺服器**：您將使用偏好的程式語言建立可運作的 MCP 伺服器，從簡單範例開始，逐步提升技能。

• **連接 AI 模型與實際工具**：您將學會如何橋接 AI 模型與真實服務，賦予應用程式強大的新功能。

• **實作安全最佳實務**：您將了解如何保持 MCP 實作的安全性，保護您的應用程式與使用者。

• **自信部署**：您將知道如何將 MCP 專案從開發階段推向生產環境，並掌握實務可行的部署策略。

• **加入 MCP 社群**：您將成為不斷壯大的開發者社群一員，共同塑造 AI 應用開發的未來。

## 必備背景知識

在深入 MCP 細節前，讓我們確保您對一些基礎概念感到熟悉。別擔心，如果您不是這些領域的專家，我們會在過程中解釋所有必要知識！
...
想像如果每家車廠使用不同形狀的加油槍——您就得為每台車準備不同轉接頭！標準化就是同意共通方式，讓事物能無縫協作。

MCP 為 AI 應用提供這種標準化。不是每個 AI 模型都需要為每個工具寫客製化程式碼，MCP 創造一種通用溝通方式。這讓開發者能一次打造工具，並讓它們與多種 AI 系統相容。
...
• **模組 2 - MCP 的安全性**：安全聽起來可能令人畏懼，但我們會展示 MCP 內建的安全機制，並教您從一開始就保護應用程式的最佳實務。
...
每個指南都包含完整程式碼範例、故障排除技巧，以及設計決策背後的解說。完成此階段後，您將擁有可運作的 MCP 實作，值得驕傲！

### 🚀 成長階段：進階概念與實務應用（模組 4-5）

掌握基礎後，您準備探索更複雜的 MCP 功能。我們將涵蓋實務實作策略、除錯技巧，以及多模態 AI 整合等進階主題。
你還將學習如何將你的 MCP 實作擴展到生產環境使用，並與像 Azure 這樣的雲端平台整合。這些模組將幫助你建立能夠應對現實需求的 MCP 解決方案。

### 🌟 精通階段：社群與專精（模組 6-11）

最後階段著重於加入 MCP 社群並專注於你最感興趣的領域。你將學習如何為開源 MCP 專案做出貢獻、實作進階的身份驗證模式，以及建立完整的資料庫整合解決方案。

模組 11 特別值得一提——它是一個包含 13 個實作實驗的完整學習路徑，教你如何建立具備 PostgreSQL 整合的生產級 MCP 伺服器。這就像是一個總結專案，將你所學全部融合！
...
| **模組 11：MCP 伺服器實作實驗** | | | |
| 11 | MCP 伺服器資料庫整合 | 13 個實驗的完整學習路徑，涵蓋 PostgreSQL 整合 | [實驗](./11-MCPServerHandsOnLabs/README.md) |
...
| 11.12 | 監控 | Application Insights、日誌、效能監控 | [實驗 11](./11-MCPServerHandsOnLabs/11-Monitoring/README.md) |
| 11.13 | 最佳實踐 | 效能優化、安全強化與生產環境建議 | [實驗 12](./11-MCPServerHandsOnLabs/12-Best-Practices/README.md) |

### 💻 範例程式專案

學習 MCP 最令人興奮的部分之一是看到你的程式技能逐步成長。我們設計的程式範例從簡單開始，隨著你理解的加深而變得更複雜。以下是我們介紹概念的方式——用易於理解但展現真實 MCP 原則的程式碼，你不僅會明白程式碼做了什麼，還會了解為什麼這樣結構化，以及它如何融入更大的 MCP 應用中。
...
提供一份完整的[學習指南](./study_guide.md)，幫助你有效瀏覽本儲存庫。這份視覺化課程地圖展示所有主題的連結，並提供如何有效使用範例專案的指引。對於喜歡從整體架構理解的視覺型學習者尤其有幫助。
...
準備好參加為期兩天的深入技術洞察、社群連結及實作學習，MCP 開發者日是一個專注於模型上下文協議（MCP）的虛擬活動 — 這是連接 AI 模型與其依賴工具的新興標準。
您可以透過我們的活動頁面註冊觀看 MCP 開發者日：https://aka.ms/mcpdevdays。
...
實用且情境驅動的開發流程
社群主導的議程與見解
無論您是剛開始接觸 MCP 或已在使用它，第 1 天都將以啟發與可行的收穫為您奠定基礎。
...
#### [➡️隨選觀看 MCP 訓練營 | 巴西葡語](https://developer.microsoft.com/en-us/reactor/series/S-1566/)
...
透過閱讀這個介紹，您已開始建立 MCP 知識基礎。您了解 MCP 是什麼、為何重要，以及這套課程如何支持您的學習旅程。這是一項重大成就，也是您在這項重要技術中成為專家的起點。

### 未來的冒險

隨著您進入各模組，請記得每位專家都曾是初學者。現在看似複雜的概念，隨著練習與應用將變得駕輕就熟。每一小步都累積成強大的能力，將伴隨您整個開發生涯。

### 您的支援網絡

您加入了一個熱衷於 MCP 且樂於助人的學習者與專家社群。無論您遇到程式挑戰或想分享突破，社群都在這裡支持您的旅程。

如果您卡關或有任何關於建立 AI 應用的問題，加入其他學習者與經驗豐富的開發者討論 MCP。這是一個歡迎提問且知識自由分享的支持性社群。
...
整套課程皆採用 MIT [LICENSE](../../LICENSE) 授權，您可以自由使用、修改及分享。這支持我們讓 MCP 知識普及給全球開發者的使命。
## 🤝 貢獻指南
...
CLA 並適當標註 PR（例如狀態檢查、評論）。只需依照機器人
...
*準備好開始您的 MCP 旅程了嗎？從 [模組 00 - MCP 介紹](./00-Introduction/README.md) 開始，踏出模型上下文協議開發世界的第一步！*
