...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-18T19:12:26+00:00",
...
把 MCP 想像成 AI 應用程式的萬用翻譯器——就像 USB 埠讓您能連接任何裝置到電腦一樣，MCP 讓 AI 模型能以標準化方式連接任何工具或服務。無論您是在打造第一個聊天機器人，還是在處理複雜的 AI 工作流程，理解 MCP 都會賦予您創造更強大且靈活應用程式的能力。

本課程設計耐心且細心，陪伴您的學習旅程。我們將從您已熟悉的簡單概念開始，並透過您喜愛的程式語言逐步建立專業知識。每一步都包含清晰的解說、實用範例，以及豐富的鼓勵。

完成這趟旅程時，您將有信心打造自己的 MCP 伺服器，將它們整合到熱門 AI 平台，並理解這項技術如何重塑 AI 開發的未來。讓我們一起開始這段令人振奮的冒險吧！

### 官方文件與規範

這些資源隨著您的理解加深而更有價值，但不必急著全部閱讀。從您最感興趣的部分開始！
- 📘 [MCP 文件](https://modelcontextprotocol.io/) – 這是您逐步教學和使用指南的首選資源。文件以初學者為對象，提供清楚的範例，讓您能依自己的節奏跟著學習。
- 📜 [MCP 規範](https://modelcontextprotocol.io/docs/) – 把它當作您的完整參考手冊。隨著課程進行，您會經常回來查閱特定細節和探索進階功能。
...
- 🌐 [MCP 社群](https://github.com/orgs/modelcontextprotocol/discussions) – 加入學習者和資深開發者的討論，這是一個支持性的社群，歡迎提問並自由分享知識。
  
## 學習目標

完成本課程後，您將感到自信且充滿熱情。以下是您將達成的目標：

• **理解 MCP 基礎**：您將掌握模型上下文協議是什麼，以及為何它正在革新 AI 應用程式的協作方式，透過類比和範例讓概念更易理解。

• **建立您的第一個 MCP 伺服器**：您將用偏好的程式語言建立一個可運作的 MCP 伺服器，從簡單範例開始，逐步提升技能。

• **連接 AI 模型與真實工具**：您將學會如何橋接 AI 模型與實際服務，賦予應用程式強大的新功能。

• **實作安全最佳實務**：您將了解如何保持 MCP 實作的安全性，保護您的應用程式和使用者。

• **自信部署**：您將知道如何將 MCP 專案從開發帶到生產環境，採用實務可行的部署策略。
...
在 MCP 中，我們有類似關係：AI 模型扮演請求資訊或動作的用戶端，MCP 伺服器則提供這些功能。就像有個幫手（伺服器）能協助 AI 執行特定任務。

### 為何標準化重要（讓事物協同運作）

想像如果每家車廠的加油槍形狀都不同——您得為每台車準備不同轉接頭！標準化就是同意共通方式，讓事物能無縫協作。

MCP 為 AI 應用提供這種標準化。不是每個 AI 模型都需要為每個工具寫客製化程式碼，MCP 創造一種通用溝通方式。這意味著開發者只需打造一次工具，就能與多種 AI 系統合作。

## 🧭 您的學習路徑概覽

您的 MCP 旅程經過精心設計，逐步建立信心與技能。每個階段介紹新概念，同時強化已學內容。
...
每個指南都包含完整程式碼範例、故障排除技巧，以及設計選擇背後的解說。完成此階段後，您將擁有可運作的 MCP 實作，值得驕傲！

### 🚀 成長階段：進階概念與實務應用（模組 4-5）

掌握基礎後，您準備探索更複雜的 MCP 功能。我們將涵蓋實務實作策略、除錯技巧，以及多模態 AI 整合等進階主題。
你亦會學習如何將你的 MCP 實作擴展至生產環境使用，並與 Azure 等雲端平台整合。這些模組讓你準備好構建能應付現實需求的 MCP 解決方案。

### 🌟 精通階段：社群與專精（模組 6-11）

最後階段著重於加入 MCP 社群及專注於你最感興趣的領域。你將學習如何為開源 MCP 專案做出貢獻、實作進階認證模式，以及構建完整的資料庫整合解決方案。

模組 11 特別值得一提——它是一個完整的 13 個實驗室的實作學習路徑，教你如何建立具備 PostgreSQL 整合的生產級 MCP 伺服器。這就像一個總結專案，將你所學全部融合！
...
| 3.10 | 進階伺服器使用 | 使用進階伺服器以實現進階功能及改善架構 | [指南](./03-GettingStarted/10-advanced/README.md) |
...
學習 MCP 最令人興奮的部分之一是看到你的程式技能逐步成長。我們設計的程式範例從簡單開始，隨著你理解加深而變得更複雜。以下是我們介紹概念的方式——用易懂但展現真實 MCP 原則的程式碼，你不僅會明白程式做什麼，還會理解為何這樣結構，以及它如何融入更大的 MCP 應用。
...
提供一份完整的[學習指南](./study_guide.md)，幫助你有效瀏覽本儲存庫。此視覺化課程地圖展示所有主題的連結，並指導如何有效使用範例專案。對於喜歡從整體視角學習的視覺型學習者尤其有幫助。
...
專為 MCP 建構者設計。我們將深入探討建立 MCP 伺服器及將 MCP 整合到 AI 工作流程中的實作策略與最佳實務。
...
- 以提示驅動的開發
- 安全最佳實務
- 使用函數、ACA 及 API 管理等組件
- 註冊表對齊與工具（1P + 3P）

如果您是開發者、工具建構者或 AI 產品策略師，這一天充滿您打造可擴展、安全且面向未來的 MCP 解決方案所需的見解。
...
如果您遇到困難或有任何關於建立 AI 應用的問題，歡迎加入 MCP 學習者與經驗豐富開發者的討論。這是一個支持性的社群，歡迎提問並自由分享知識。
...
您的 MCP 冒險現在開始！從模組 0 開始，深入您的第一個 MCP 實作體驗，或探索範例專案看看您將會打造什麼。請記得——每位專家都是從您現在的位置開始，憑藉耐心與練習，您將驚訝於自己的成就。
...
整套課程皆採用 MIT [授權條款](../../LICENSE)，您可以自由使用、修改及分享。這支持我們讓 MCP 知識普及給全球開發者的使命。
