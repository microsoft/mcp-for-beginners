...
> **Current standard:** Dis document dey reflect
> [MCP Specification 2026-07-28](https://modelcontextprotocol.io/specification/2026-07-28/)
> and di official
> [MCP Security Best Practices](https://modelcontextprotocol.io/specification/2026-07-28/basic/security_best_practices).

Di Model Context Protocol (MCP) don mature well well wit better security controls wey dey cover both traditional software security and AI-specific wahala dem. Dis document dey provide full security controls for secure MCP implementations wey gree wit di OWASP MCP Top 10 framework.

## 🏔️ Hands-On Security Training

For correct, hands-on security implementation experience, we recommend di **[MCP Security Summit Workshop (Sherpa)](https://azure-samples.github.io/sherpa/)** - na one full guided journey to secure MCP servers for Azure using di "vulnerable → exploit → fix → validate" method.

All di security controls for dis document gree wit di **[OWASP MCP Azure Security Guide](https://microsoft.github.io/mcp-azure-security-guide/)**, wey dey provide reference architectures and Azure-specific implementation guidance for di OWASP MCP Top 10 risks dem.
...
>
> **REQUIRED**: MCP servers wey dey implement authorization **MUST** check ALL inbound requests
>
> **MANDATORY**: MCP proxy servers wey dey use static third-party client ID
> **MUST** get consent for each MCP client before dem forward authorization
...
**MCP Specification `2026-07-28`** allow MCP servers make dem fit delegate
authentication to external identity providers. Authorization for HTTP
transports e dey evaluate per request; local stdio servers dey get credentials
...
**Security Benefits:**
1. **Eliminates Custom Authentication Risks**: E reduce vulnerability chance by avoiding custom authentication implementations
2. **Enterprise-Grade Security**: E use well established identity providers like Microsoft Entra ID wit better security features
3. **Centralized Identity Management**: E dey simplify user lifecycle management, access control, and compliance auditing
...
- **Client Registration**: Prefer Client ID Metadata Documents or
  ...
  compatibility
- **Token Audience Validation**: Make sure all tokens dey explicitly issued for di MCP server
- **Issuer Verification**: Validate token issuer make e match di expected identity provider
...
- **Scope Validation**: Make sure tokens get right permissions for requested operations
...
**Token passthrough na expressly forbidden** for di MCP Authorization Specification because e get serious security risks:

**Security Risks Addressed:**
- **Control Circumvention**: E dey bypass important security controls like rate limiting, request validation, and traffic monitoring
- **Accountability Breakdown**: E make client identification impossible, e spoil audit trails and incident investigation
- **Proxy-Based Exfiltration**: E allow bad people use servers as proxies for unauthorized data access
- **Trust Boundary Violations**: E break downstream service trust assumptions about token origins
- **Lateral Movement**: Compromised tokens for many services dey allow bigger attack expansion
...
**Best Practices:**
- **Short-Lived Tokens**: Make exposure time short by rotating tokens often
- **Just-in-Time Issuance**: Issue tokens only wen dem need am for specific operations
- **Secure Storage**: Use hardware security modules (HSMs) or secure key vaults
- **Token Binding**: Validate token audience and issuer for di correct MCP
...
- **Cross-user Reuse**: A stolen handle dey use am wit different identity
- **Implicit Authorization**: Possession of handle e dey wrongly treat as
  proof of access
...
- **Credential Handling**: Send and validate authorization every HTTP request
...
MCP `2026-07-28` na stateless for di protocol layer. Applications fit still
keep state by returning explicit handle from one tool call and accept am
as normal argument on later calls.

- Store state separate from any one transport connection.
- Bind state handles to authenticated principal server-side.
- Treat handle as a name, no be bearer credential.
...
> **Important:** Dem security controls dey represent MCP Specification
> `2026-07-28`. Make you always check am for
> [current official documentation](https://modelcontextprotocol.io/specification/2026-07-28/)
> as standards still dey change.

## Wetin Next

- Return go: [Security Module Overview](./README.md)
- Continue go: [Module 3: Getting Started](../03-GettingStarted/README.md)
