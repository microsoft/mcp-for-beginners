...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-12T16:36:17+00:00",
...
歡迎展開你的 Model Context Protocol 之旅！如果你曾經好奇 AI 應用程式如何與不同工具和服務溝通，你即將發現這個優雅的解決方案，正改變開發者構建智慧系統的方式。

把 MCP 想像成 AI 應用程式的萬用翻譯器——就像 USB 埠讓你能將任何裝置連接到電腦一樣，MCP 讓 AI 模型能以標準化方式連接任何工具或服務。無論你是在打造第一個聊天機器人，還是在處理複雜的 AI 工作流程，理解 MCP 都會賦予你創造更強大且靈活應用程式的能力。

本課程設計時充分考慮你的學習歷程。我們將從你已熟悉的簡單概念開始，並透過你喜愛的程式語言逐步建立專業知識。每一步都包含清晰的解說、實用範例，以及大量鼓勵。

完成這趟旅程時，你將有信心打造自己的 MCP 伺服器，將它們整合到熱門 AI 平台，並理解這項技術如何重塑 AI 開發的未來。讓我們一起開始這段令人振奮的冒險吧！

### 官方文件與規範

這些資源隨著你的理解加深會越來越有價值，但不必急著全部閱讀。先從你最感興趣的部分開始！
- 📘 [MCP 文件](https://modelcontextprotocol.io/) – 這是你的逐步教學與使用指南首選資源。文件以初學者為對象，提供清楚的範例，讓你能依自己的節奏跟著學習。
- 📜 [MCP 規範](https://modelcontextprotocol.io/docs/) – 把它當作你的完整參考手冊。隨著課程進行，你會經常回來查閱特定細節與探索進階功能。
- 📜 [原始 MCP 規範](https://spec.modelcontextprotocol.io/) – 包含更多技術細節，對進階實作有幫助。需要時可參考，初學時不必擔心。
- 🧑‍💻 [MCP GitHub 儲存庫](https://github.com/modelcontextprotocol) – 這裡有多種程式語言的 SDK、工具和程式碼範例。就像一座實用範例與現成元件的寶庫。
- 🌐 [MCP 社群](https://github.com/orgs/modelcontextprotocol/discussions) – 加入其他學習者與資深開發者的討論。這是一個支持性的社群，歡迎提問並自由分享知識。
  
## 學習目標

完成本課程後，你會對新技能充滿信心與熱情。你將達成以下目標：

• **理解 MCP 基礎**：你會明白 Model Context Protocol 是什麼，為何它正在革新 AI 應用程式的協作方式，並透過類比與範例讓概念更易理解。

• **建立你的第一個 MCP 伺服器**：你將用偏好的程式語言建立一個可運作的 MCP 伺服器，從簡單範例開始，逐步提升技能。

• **連接 AI 模型與真實工具**：你會學會如何橋接 AI 模型與實際服務，賦予應用程式強大的新功能。

• **實作安全最佳實務**：你將了解如何保持 MCP 實作的安全性，保護你的應用程式與使用者。

• **自信部署**：你會知道如何將 MCP 專案從開發帶到生產環境，並掌握實用的部署策略。

• **加入 MCP 社群**：你將成為不斷壯大的開發者社群一員，共同塑造 AI 應用開發的未來。

## 必備背景知識

在深入 MCP 細節前，讓我們確保你對一些基礎概念感到自在。別擔心，如果你不是這些領域的專家——我們會在過程中解釋你需要知道的一切！

### 理解協定（基礎）

把協定想像成對話的規則。當你打電話給朋友，雙方都知道接聽時要說「哈囉」，輪流說話，結束時說「再見」。電腦程式也需要類似規則才能有效溝通。

MCP 是一種協定——一套約定好的規則，幫助 AI 模型和應用程式與工具和服務進行有效「對話」。就像對話規則讓人類溝通更順暢，MCP 讓 AI 應用溝通更可靠且強大。

### 用戶端-伺服器關係（程式如何協作）

你每天都在使用用戶端-伺服器關係！當你用瀏覽器（用戶端）造訪網站時，你連接到會傳送頁面內容的網頁伺服器。瀏覽器知道如何請求資訊，伺服器知道如何回應。

在 MCP 中，我們有類似關係：AI 模型扮演請求資訊或動作的用戶端，而 MCP 伺服器提供這些功能。就像有個貼心助理（伺服器）讓 AI 可以請求執行特定任務。

### 為何標準化重要（讓事物能協同運作）

想像如果每家車廠的加油槍形狀都不同——你得為每台車準備不同轉接頭！標準化就是同意共通方式，讓事物能無縫協作。

MCP 為 AI 應用提供這種標準化。不是每個 AI 模型都需要客製化程式碼來配合每個工具，MCP 創造一種通用溝通方式。這意味著開發者只需打造一次工具，就能與多種 AI 系統合作。

## 🧭 你的學習路徑概覽

你的 MCP 旅程經過精心設計，逐步建立信心與技能。每個階段介紹新概念，同時強化已學內容。

### 🌱 基礎階段：理解基本概念（模組 0-2）

冒險從這裡開始！我們會用熟悉的類比和簡單範例介紹 MCP 概念。你會了解 MCP 是什麼、為何存在，以及它如何融入 AI 開發的宏觀世界。

• **模組 0 - MCP 簡介**：我們將探索 MCP 是什麼，為何對現代 AI 應用如此重要。你會看到 MCP 實際運作的範例，並理解它如何解決開發者常見問題。

• **模組 1 - 核心概念說明**：這裡你會學到 MCP 的基本構成要素。我們會用大量類比和視覺範例，確保這些概念自然且易懂。

• **模組 2 - MCP 的安全性**：安全聽起來可能令人畏懼，但我們會展示 MCP 內建的安全功能，並教你從一開始就保護應用程式的最佳實務。

### 🔨 建構階段：打造你的第一個實作（模組 3）

真正的樂趣現在開始！你將親手打造 MCP 伺服器和用戶端。別擔心——我們會從簡單開始，逐步引導你完成每一步。

本模組包含多個實作指南，讓你用偏好的程式語言練習。你會建立第一個伺服器，打造連接它的用戶端，甚至整合熱門開發工具如 VS Code。

每個指南都包含完整程式碼範例、故障排除技巧，以及設計選擇背後的解說。完成此階段後，你將擁有可運作的 MCP 實作，值得驕傲！

### 🚀 成長階段：進階概念與實務應用（模組 4-5）

掌握基礎後，你準備探索更複雜的 MCP 功能。我們將涵蓋實務實作策略、除錯技巧，以及多模態 AI 整合等進階主題。
你還將學習如何將你的 MCP 實作擴展到生產環境使用，並與 Azure 等雲端平台整合。這些模組將幫助你建立能夠應對現實需求的 MCP 解決方案。

### 🌟 精通階段：社群與專精（模組 6-11）

最後階段著重於加入 MCP 社群並專注於你最感興趣的領域。你將學習如何為開源 MCP 專案做出貢獻，實作進階的身份驗證模式，並建立完整的資料庫整合解決方案。

模組 11 特別值得一提——它是一個包含 13 個實作實驗的完整學習路徑，教你如何建立具備 PostgreSQL 整合的生產級 MCP 伺服器。這就像是一個總結專案，將你所學全部串連起來！
...
| 01 | Core Concepts Explained | 深入探討 MCP 核心概念 | [Read more](./01-CoreConcepts/README.md) |
| 02 | Security in MCP | 安全威脅與最佳實踐 | [Read more](./02-Security/README.md) |
| 03 | Getting Started with MCP | 環境設定、基本伺服器/客戶端、整合 | [Read more](./03-GettingStarted/README.md) |
...
| 3.2 | First Client | 開發基本 MCP 客戶端 | [Guide](./03-GettingStarted/02-client/README.md) |
...
| 3.6 | HTTP Streaming | 在 MCP 中實作 HTTP 串流 | [Guide](./03-GettingStarted/06-http-streaming/README.md) |
| 3.7 | AI Toolkit | 使用 AI Toolkit 搭配 MCP | [Guide](./03-GettingStarted/07-aitk/README.md) |
...
| 3.10 | Advanced server usage | 使用進階伺服器以利用進階功能及改善架構 | [Guide](./03-GettingStarted/10-advanced/README.md) |
| 3.11 | Simple auth | 從頭開始介紹身份驗證與 RBAC | [Guide](./03-GettingStarted/11-simple-auth/README.md) |
...
| 05 | Advanced Topics in MCP | 多模態 AI、擴展、企業應用 | [Read more](./05-AdvancedTopics/README.md) |
| 5.1 | Azure Integration | MCP 與 Azure 整合 | [Guide](./05-AdvancedTopics/mcp-integration/README.md) |
| 5.2 | Multi-modality | 多模態處理 | [Guide](./05-AdvancedTopics/mcp-multi-modality/README.md) |
| 5.3 | OAuth2 Demo | 實作 OAuth2 身份驗證 | [Guide](./05-AdvancedTopics/mcp-oauth2-demo/README.md) |
| 5.4 | Root Contexts | 理解並實作根上下文 | [Guide](./05-AdvancedTopics/mcp-root-contexts/README.md) |
| 5.5 | Routing | MCP 路由策略 | [Guide](./05-AdvancedTopics/mcp-routing/README.md) |
| 5.6 | Sampling | MCP 中的取樣技術 | [Guide](./05-AdvancedTopics/mcp-sampling/README.md) |
...
| 5.9 | Web Search | 實作網路搜尋功能 | [Guide](./05-AdvancedTopics/web-search-mcp/README.md) |
...
| 5.12 | Entra ID Auth | 使用 Microsoft Entra ID 進行身份驗證 | [Guide](./05-AdvancedTopics/mcp-security-entra/README.md) |
| 5.13 | Foundry Integration | 與 Azure AI Foundry 整合 | [Guide](./05-AdvancedTopics/mcp-foundry-agent-integration/README.md) |
| 5.14 | Context Engineering | 有效上下文工程技術 | [Guide](./05-AdvancedTopics/mcp-contextengineering/README.md) |
| 5.15 | MCP Custom Transport | 自訂傳輸實作 | [Guide](./05-AdvancedTopics/mcp-transport/README.md) |
| **Module 6-10: Community & Best Practices** | | | |
| 06 | Community Contributions | 如何為 MCP 生態系做出貢獻 | [Guide](./06-CommunityContributions/README.md) |
| 07 | Insights from Early Adoption | 實際應用案例分享 | [Guide](./07-LessonsFromEarlyAdoption/README.md) |
| 08 | Best Practices for MCP | 效能、容錯、韌性最佳實踐 | [Guide](./08-BestPractices/README.md) |
| 09 | MCP Case Studies | 實務案例分析 | [Guide](./09-CaseStudy/README.md) |
...
| 11 | MCP Server Database Integration | 13 個實驗組成的完整 PostgreSQL 整合實作路徑 | [Labs](./11-MCPServerHandsOnLabs/README.md) |
| 11.1 | Introduction | MCP 與資料庫整合及零售分析案例概述 | [Lab 00](./11-MCPServerHandsOnLabs/00-Introduction/README.md) |
| 11.2 | Core Architecture | 理解 MCP 伺服器架構、資料庫層與安全模式 | [Lab 01](./11-MCPServerHandsOnLabs/01-Architecture/README.md) |
| 11.3 | Security & Multi-Tenancy | 行級安全、多租戶身份驗證與資料存取 | [Lab 02](./11-MCPServerHandsOnLabs/02-Security/README.md) |
| 11.4 | Environment Setup | 開發環境設定、Docker、Azure 資源 | [Lab 03](./11-MCPServerHandsOnLabs/03-Setup/README.md) |
| 11.5 | Database Design | PostgreSQL 設定、零售架構設計與範例資料 | [Lab 04](./11-MCPServerHandsOnLabs/04-Database/README.md) |
| 11.6 | MCP Server Implementation | 建立具資料庫整合的 FastMCP 伺服器 | [Lab 05](./11-MCPServerHandsOnLabs/05-MCP-Server/README.md) |
| 11.7 | Tool Development | 建立資料庫查詢工具與架構檢視 | [Lab 06](./11-MCPServerHandsOnLabs/06-Tools/README.md) |
| 11.8 | Semantic Search | 使用 Azure OpenAI 與 pgvector 實作向量嵌入 | [Lab 07](./11-MCPServerHandsOnLabs/07-Semantic-Search/README.md) |
| 11.9 | Testing & Debugging | 測試策略、除錯工具與驗證方法 | [Lab 08](./11-MCPServerHandsOnLabs/08-Testing/README.md) |
| 11.10 | VS Code Integration | 設定 VS Code MCP 整合與 AI 聊天使用 | [Lab 09](./11-MCPServerHandsOnLabs/09-VS-Code/README.md) |
| 11.11 | Deployment Strategies | Docker 部署、Azure Container Apps 與擴展考量 | [Lab 10](./11-MCPServerHandsOnLabs/10-Deployment/README.md) |
| 11.12 | Monitoring | Application Insights、日誌與效能監控 | [Lab 11](./11-MCPServerHandsOnLabs/11-Monitoring/README.md) |
| 11.13 | Best Practices | 效能優化、安全強化與生產環境建議 | [Lab 12](./11-MCPServerHandsOnLabs/12-Best-Practices/README.md) |

### 💻 範例程式專案

學習 MCP 最令人興奮的部分之一是看到你的程式技能逐步成長。我們設計的程式範例從簡單開始，隨著你理解的加深而變得更複雜。以下是我們介紹概念的方式——用易於理解但展現真實 MCP 原則的程式碼，你不僅會明白程式做了什麼，還會理解為什麼這樣結構化，以及它如何融入更大的 MCP 應用中。
...
提供一份完整的[學習指南](./study_guide.md)，幫助你有效瀏覽本儲存庫。這份視覺化課程地圖展示所有主題的連結，並指導你如何有效使用範例專案。對於喜歡從整體視角學習的視覺型學習者尤其有幫助。
...
準備好參加為期兩天的深入技術洞察、社群連結及實作學習，MCP 開發日是一個專注於模型上下文協議（MCP）的虛擬活動 — 這是連接 AI 模型與其依賴工具的新興標準。
...
專注於賦能開發者在開發流程中使用 MCP，並慶祝精彩的 MCP 社群。我們將與 Arcade、Block、Okta 和 Neon 等社群成員及合作夥伴一同探討他們如何與微軟合作，塑造一個開放且可擴展的 MCP 生態系統。實際示範涵蓋 VS Code、Visual Studio、GitHub Copilot 及熱門社群工具
實用且情境驅動的開發流程
社群主導的議程與見解
無論您是剛開始接觸 MCP 還是已經在使用它，第 1 天都將帶來啟發與可行的收穫。

#### [第 2 天：自信打造 MCP 伺服器](https://developer.microsoft.com/en-us/reactor/series/S-1563/)

專為 MCP 建構者設計。我們將深入探討建立 MCP 伺服器及將 MCP 整合到 AI 工作流程中的實作策略與最佳實踐。
...
如果您是開發者、工具建構者或 AI 產品策略師，這一天充滿您打造可擴展、安全且面向未來的 MCP 解決方案所需的洞見。

### MCP 速成營 2025 年 8 月
透過密集的影片課程學習如何建立 MCP 伺服器、整合 VS Code，並根據 MCP 初學者課程內容在 Azure 上專業部署。帶著實用技能離開，這是大型企業已經在使用的技術。

#### [➡️隨選觀看 MCP 速成營 | 英文](https://developer.microsoft.com/en-us/reactor/series/s-1568/)
#### [➡️隨選觀看 MCP 速成營 | 巴西葡萄牙語](https://developer.microsoft.com/en-us/reactor/series/S-1566/)
#### [➡️隨選觀看 MCP 速成營 | 西班牙語](https://developer.microsoft.com/en-us/reactor/series/S-1567/)

### 一起用 C# 學 MCP - 教學系列
讓我們一起了解模型上下文協議（MCP），這是一個前沿框架，旨在標準化 AI 模型與客戶端應用程式之間的互動。透過這個適合初學者的課程，我們將介紹 MCP 並引導您建立第一個 MCP 伺服器。
...
恭喜！您剛踏出令人振奮的第一步，這將擴展您的程式設計能力，並連結您至 AI 開發的最前沿。
...
隨著您進入各模組，請記得每位專家都曾是初學者。現在看似複雜的概念，隨著練習與應用將變得駕輕就熟。每一小步都累積成強大的能力，將在您的開發生涯中助您一臂之力。

### 您的支援網絡

您正加入一個熱衷於 MCP 且樂於助人的學習者與專家社群。無論您遇到程式挑戰還是想分享突破，社群都在這裡支持您的旅程。

如果您卡關或有任何關於建立 AI 應用的問題，加入與其他學習者及經驗豐富開發者的 MCP 討論。這是一個歡迎提問且知識自由分享的支持性社群。
...
您的 MCP 冒險現在開始！從模組 0 開始，深入您的第一個 MCP 實作體驗，或探索範例專案看看您將會打造什麼。記住 — 每位專家都是從您現在的位置開始，憑藉耐心與練習，您將驚訝於自己的成就。
...
貢獻流程設計為友善且支持性。大多數貢獻需要簽署貢獻者授權協議（CLA），但自動化工具會順利引導您完成流程。

## 📜 開源學習

整套課程皆在 MIT [LICENSE](../../LICENSE) 授權下提供，您可以自由使用、修改及分享。這支持我們讓 MCP 知識普及給全球開發者的使命。

## 🤝 貢獻指南

本專案歡迎貢獻與建議。大多數貢獻需您同意一份貢獻者授權協議（CLA），聲明您有權利且確實授權我們使用您的貢獻。詳情請參閱 <https://cla.opensource.microsoft.com>。

當您提交拉取請求時，CLA 機器人會自動判斷您是否需要提供 CLA，並適當標註 PR（例如狀態檢查、評論）。請依照機器人指示操作。您只需在所有使用我們 CLA 的倉庫中完成一次。
...
*準備好開始您的 MCP 旅程了嗎？從 [模組 00 - MCP 介紹](./00-Introduction/README.md) 開始，踏入模型上下文協議開發的世界！*
