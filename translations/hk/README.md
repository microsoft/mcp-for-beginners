...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-18T19:18:33+00:00",
  "source_file": "README.md",
  "language_code": "hk"
}
-->
![MCP-for-beginners](../../translated_images/hk/mcp-beginners.2ce2b317996369ff66c5b72e25eff9d4288ab2741fc70c0b4e523d1ae1e249fd.png) 
...
歡迎展開你的 Model Context Protocol 之旅！如果你曾好奇 AI 應用程式如何與不同工具和服務溝通，你即將發現這個優雅的解決方案，正改變開發者打造智慧系統的方式。
...
本課程設計耐心且細心，陪伴你的學習旅程。我們將從你已熟悉的簡單概念開始，並透過你喜愛的程式語言逐步建立專業知識。每個步驟都包含清晰的解說、實用範例，以及大量鼓勵。

完成這趟旅程時，你將有信心打造自己的 MCP 伺服器，將它們整合到熱門 AI 平台，並理解這項技術如何重塑 AI 開發的未來。讓我們一起開始這段令人振奮的冒險吧！

### 官方文件與規範

這些資源會隨著你的理解加深而更有價值，但不必急著全部閱讀。先從你最感興趣的部分開始！
...
- 📜 [原始 MCP 規範](https://modelcontextprotocol.io/specification/versioning) – 包含更多技術細節，對進階實作有幫助。需要時可參考，初學時不必擔心。
- 🧑‍💻 [MCP GitHub 倉庫](https://github.com/modelcontextprotocol) – 這裡有多種程式語言的 SDK、工具和程式碼範例。就像一座實用範例與現成元件的寶庫。
- 🌐 [MCP 社群](https://github.com/orgs/modelcontextprotocol/discussions) – 加入其他學習者與經驗豐富的開發者，一起討論 MCP。這是一個支持性強、歡迎提問且自由分享知識的社群。
...
• **打造你的第一個 MCP 伺服器**：你將用自己偏好的程式語言建立一個可運作的 MCP 伺服器，從簡單範例開始，逐步提升技能。
...
在深入 MCP 細節前，讓我們先確保你對一些基礎概念感到自在。別擔心，如果你不是這些領域的專家，我們會在過程中解釋所有你需要知道的內容！

### 理解協定（基礎）

把協定想像成對話的規則。當你打電話給朋友，雙方都知道接聽時要說「哈囉」、輪流說話，結束時說「再見」。電腦程式也需要類似規則才能有效溝通。

MCP 是一種協定——一套約定好的規則，幫助 AI 模型與應用程式與工具和服務進行有效「對話」。就像對話規則讓人類溝通更順暢，MCP 讓 AI 應用溝通更可靠且強大。
...
想像如果每家車廠的加油槍形狀都不同——你就得為每台車準備不同轉接頭！標準化就是同意共通方式，讓事物能無縫協作。

MCP 為 AI 應用提供這種標準化。不是每個 AI 模型都要寫客製化程式碼來配合每個工具，MCP 創造一種通用溝通方式。這讓開發者能一次打造工具，並讓它們與多種 AI 系統兼容。

## 🧭 你的學習路徑概覽

你的 MCP 旅程經過精心設計，逐步建立信心與技能。每個階段介紹新概念，同時強化你已學的內容。

### 🌱 基礎階段：理解基本概念（模組 0-2）

冒險從這裡開始！我們會用熟悉的類比和簡單範例介紹 MCP 概念。你會了解 MCP 是什麼、為何存在，以及它如何融入 AI 開發的宏觀世界。

• **模組 0 - MCP 介紹**：我們將從探索 MCP 是什麼及其對現代 AI 應用的重要性開始。你會看到 MCP 實際運作的範例，並理解它如何解決開發者常見問題。
...
真正的樂趣從這裡開始！你將親手打造 MCP 伺服器和用戶端。別擔心——我們會從簡單開始，逐步引導你完成每一步。
...
你還將學習如何將你的 MCP 實作擴展到生產環境使用，並與像 Azure 這樣的雲端平台整合。這些模組將幫助你準備構建能夠應對現實需求的 MCP 解決方案。

### 🌟 精通階段：社群與專精（模組 6-11）

最後階段專注於加入 MCP 社群並專精你最感興趣的領域。你將學習如何為開源 MCP 專案做出貢獻，實作進階的身份驗證模式，並構建完整的資料庫整合解決方案。

模組 11 特別值得一提——它是一個完整的 13 個實驗室的實作學習路徑，教你如何建立具備 PostgreSQL 整合的生產級 MCP 伺服器。這就像是一個總結專案，將你所學的一切串連起來！
...
| **模組 11：MCP 伺服器實作實驗室** | | | |
| 11 | MCP 伺服器資料庫整合 | 13 個實驗室的完整 PostgreSQL 整合實作學習路徑 | [實驗室](./11-MCPServerHandsOnLabs/README.md) |
...
| 11.3 | 安全與多租戶 | 行級安全、身份驗證與多租戶資料存取 | [實驗室 02](./11-MCPServerHandsOnLabs/02-Security/README.md) |
...
| 11.13 | 最佳實踐 | 效能優化、安全強化與生產環境建議 | [實驗室 12](./11-MCPServerHandsOnLabs/12-Best-Practices/README.md) |

### 💻 範例程式專案

學習 MCP 最令人興奮的部分之一是看到你的程式技能逐步成長。我們設計的程式範例從簡單開始，隨著你理解的加深而變得更複雜。以下是我們介紹概念的方式——用易於理解但展現真實 MCP 原則的程式碼，你不僅會了解程式碼做了什麼，還會明白為什麼這樣結構化，以及它如何融入更大的 MCP 應用中。
...
提供一份完整的 [學習指南](./study_guide.md)，幫助你有效瀏覽本儲存庫。這份視覺化課程地圖展示所有主題的連結，並提供如何有效使用範例專案的指引。對於喜歡從整體視角學習的視覺型學習者特別有幫助。
...
我們維護一份詳細的 [更新日誌](./changelog.md)，追蹤課程材料的所有重大更新，讓你隨時掌握最新改進與新增內容。
- 新增內容
- 結構調整
- 功能改進
- 文件更新
...
準備好參加為期兩天的深入技術洞察、社群連結及實作學習的 MCP 開發日虛擬活動，專注於 Model Context Protocol (MCP) — 這個連接 AI 模型與其依賴工具的新興標準。
您可以在我們的活動頁面註冊觀看 MCP 開發日：https://aka.ms/mcpdevdays。

#### [第 1 天：MCP 生產力、開發工具與社群：](https://developer.microsoft.com/en-us/reactor/series/S-1563/)

專注於賦能開發者在開發工作流程中使用 MCP，並慶祝精彩的 MCP 社群。我們將與 Arcade、Block、Okta 和 Neon 等社群成員及合作夥伴一同探討他們如何與微軟合作，塑造一個開放且可擴展的 MCP 生態系統。實際示範涵蓋 VS Code、Visual Studio、GitHub Copilot 及熱門社群工具
實用且情境驅動的開發工作流程
社群主導的議程與見解
無論您是剛開始接觸 MCP 還是已經在使用它，第 1 天都將以啟發與可行的收穫為您奠定基礎。

#### [第 2 天：自信打造 MCP 伺服器](https://developer.microsoft.com/en-us/reactor/series/S-1563/)

專為 MCP 建構者設計。我們將深入探討建立 MCP 伺服器的實作策略與最佳實務，並將 MCP 整合到您的 AI 工作流程中。
...
### MCP 2025 年 8 月訓練營
透過密集的影片課程學習如何建立 MCP 伺服器、與 VS Code 整合，並根據 MCP 初學者課程內容在 Azure 上專業部署。帶著實用技能離開，這是許多大型企業已在使用的技術。
...
### 一起用 C# 學 MCP - 教學系列
讓我們一起了解 Model Context Protocol (MCP)，這是一個前沿框架，旨在標準化 AI 模型與客戶端應用程式之間的互動。透過這個適合初學者的課程，我們將介紹 MCP 並引導您建立第一個 MCP 伺服器。
...
透過閱讀這個介紹，您已開始建立 MCP 知識基礎。您了解 MCP 是什麼、為何重要，以及這套課程如何支持您的學習旅程。這是一項重要成就，也是您在這項重要技術中成為專家的起點。

### 未來的冒險

隨著您進入各模組，請記得每位專家都曾是初學者。現在看似複雜的概念，隨著練習與應用將變得駕輕就熟。每一小步都將累積成強大的能力，伴隨您整個開發生涯。

### 您的支援網絡

您將加入一個熱衷於 MCP 且樂於助人的學習者與專家社群。無論您遇到程式挑戰還是想分享突破，社群都在這裡支持您的旅程。

如果您卡關或有任何關於建立 AI 應用的問題，歡迎加入與其他學習者及經驗豐富開發者的 MCP 討論。這是一個支持性強、歡迎提問且知識自由分享的社群。
...
您的 MCP 冒險現在開始！從模組 0 開始，深入您的第一個 MCP 實作體驗，或探索範例專案看看您將會打造什麼。記住 — 每位專家都是從您現在的位置開始，憑著耐心與練習，您將驚訝於自己的成就。
...
貢獻流程設計為友善且支持性強。大多數貢獻需要簽署貢獻者授權協議 (CLA)，但自動化工具會順利引導您完成流程。
...
CLA 並適當標註 PR（例如狀態檢查、留言）。只需依照機器人
指示操作。您在所有使用我們 CLA 的倉庫中只需執行一次。
...
*準備好開始您的 MCP 旅程了嗎？從 [模組 00 - MCP 介紹](./00-Introduction/README.md) 開始，踏入 Model Context Protocol 開發的世界！*
