...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-19T09:11:49+00:00",
...
Model Context Protocol 引入了超越傳統軟件安全的獨特安全挑戰。這些實踐涵蓋基礎安全需求及 MCP 特有威脅，包括提示注入、工具中毒、會話劫持、混淆代理問題及令牌透傳漏洞。
...
>  
> **必須**：使用靜態客戶端 ID 的 MCP 代理伺服器**必須**為每個動態註冊的客戶端取得用戶同意
...
**身份驗證與授權控制：**  
   - **嚴格授權審查**：全面審核 MCP 伺服器授權邏輯，確保僅授權預期用戶與客戶端存取資源  
...
**受保護的令牌存儲：**  
   - 使用 Azure Key Vault 或類似安全憑證庫存放所有秘密  
   - 實施令牌靜態與傳輸加密  
   - 定期輪替憑證並監控未授權存取  
...
**安全會話實踐：**  
   - **密碼學安全的會話 ID**：使用安全隨機數生成器產生非決定性會話 ID  
   - **用戶綁定**：使用 `<user_id>:<session_id>` 格式將會話 ID 綁定至用戶身份，防止跨用戶會話濫用  
   - **會話生命週期管理**：實施適當的過期、輪替與失效機制，限制漏洞窗口  
...
**提示注入防禦：**  
   - **Microsoft 提示盾牌**：部署 AI 提示盾牌以進行惡意指令的高級偵測與過濾  
   - **輸入消毒**：驗證並消毒所有輸入，防止注入攻擊及混淆代理問題  
   - **內容邊界**：使用分隔符與數據標記系統區分可信指令與外部內容  

**工具中毒防範：**  
   - **工具元資料驗證**：實施工具定義完整性檢查並監控異常變更  
   - **動態工具監控**：監控執行時行為並設置異常執行模式警報  
   - **審批工作流程**：工具修改與能力變更需用戶明確批准  

## 4. **存取控制與權限**

**最小權限原則：**  
   - 僅授予 MCP 伺服器執行預期功能所需的最低權限  
   - 實施基於角色的存取控制（RBAC）並細分權限  
   - 定期審查權限並持續監控權限升級  
...
**內容安全實作：**  
   ...
   - **行為分析**：實施執行時行為監控，偵測 MCP 伺服器與工具執行異常  
   - **全面日誌記錄**：記錄所有身份驗證嘗試、工具調用及安全事件，並使用安全防篡改存儲  

**持續監控：**  
   - 即時警報可疑模式及未授權存取嘗試  
   - 與 SIEM 系統整合以集中管理安全事件  
   - 定期進行 MCP 實作的安全審計與滲透測試  

## 6. **供應鏈安全**

**元件驗證：**  
   - **依賴掃描**：對所有軟件依賴與 AI 元件使用自動化漏洞掃描  
   - **來源驗證**：驗證模型、資料來源及外部服務的來源、授權及完整性  
   - **簽名套件**：使用加密簽名套件並於部署前驗證簽名  

**安全開發管線：**  
   - **GitHub 進階安全**：實施秘密掃描、依賴分析及 CodeQL 靜態分析  
   - **CI/CD 安全**：在自動部署管線中整合安全驗證  
   - **工件完整性**：對部署工件與配置實施加密驗證  

## 7. **OAuth 安全與混淆代理防範**

**OAuth 2.1 實作：**  
   - **PKCE 實作**：所有授權請求使用 Proof Key for Code Exchange (PKCE)  
   - **明確同意**：為每個動態註冊客戶端取得用戶同意，防止混淆代理攻擊  
...
**快速響應能力：**  
   - **自動化響應**：實施自動化系統進行憑證輪替與威脅遏制  
   - **回滾程序**：能快速回復至已知良好配置與元件  
   - **取證能力**：詳細審計軌跡與日誌以供事件調查  
...
**法規遵循：**  
   ...
   - 實施 AI 資料處理的資料分類與隱私控制  
   - 維護完整文件以供合規審計  

**變更管理：**  
   - 對所有 MCP 系統修改進行正式安全審查  
   - 配置變更實施版本控制與審批工作流程  
   - 定期合規評估與差距分析  
...
**零信任架構：**  
   - **永不信任，持續驗證**：持續驗證用戶、設備與連線  
   - **微分段**：細粒度網路控制隔離各 MCP 元件  
   - **條件存取**：基於風險的存取控制，依當前情境與行為調整  

**執行時應用保護：**  
   - **執行時應用自我保護 (RASP)**：部署 RASP 技術進行即時威脅偵測  
...
**保持最新：**  
   ...
   - **安全社群參與**：積極參與 MCP 安全社群與漏洞揭露計劃  

**適應性安全：**  
   - **機器學習安全**：使用 ML 基異常偵測識別新型攻擊模式  
   - **預測性安全分析**：實施預測模型以主動識別威脅  
...
> **安全通知**：MCP 安全實踐快速演進。實作前請務必核對最新的 [MCP 規範](https://spec.modelcontextprotocol.io/) 及 [官方安全文件](https://modelcontextprotocol.io/specification/2025-11-25/basic/security_best_practices)。
