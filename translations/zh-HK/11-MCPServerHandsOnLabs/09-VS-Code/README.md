...
> `2025-11-25` 握手。MCP `2026-07-28` 移除了初始化握手；
> 遷移此範例時，請使用支援每次請求元資料及 `server/discover`
> 的主機與 SDK。

## 🎯 本實驗室涵蓋內容

本實驗室提供如何整合 MCP 伺服器與 VS Code，透過 AI Chat 啟用自然語言查詢的完整指導。您將學習如何配置 VS Code 以最佳化 MCP 使用，除錯伺服器連線，並充分利用 AI 協助的資料庫互動能力。

## 概覽

VS Code 的 MCP 整合改變了開發者以自然語言與資料庫及 API 互動的方式。將您的零售 MCP 伺服器連接至 VS Code Chat，您可以透過對話式 AI 智能查詢銷售數據、產品目錄及商業分析。

此整合使開發者能提出像「顯示本月熱賣產品排行」或「找出超過 90 天未購買的客戶」等問題，並獲得具結構化數據的回應，而不需撰寫 SQL 查詢。

## 學習目標

完成本實驗室後，您將能：

- <strong>配置</strong> VS Code MCP 設定以適應您的零售伺服器
- <strong>整合</strong> MCP 伺服器至 VS Code AI Chat 功能
- <strong>除錯</strong> MCP 伺服器連線並排查問題
- <strong>優化</strong> 自然語言查詢模式以提升結果
...
```typescript
// src/extension.ts - 自訂 MCP 零售擴充套件
...
✅ **VS Code MCP 配置**：完整設定以達到最佳 MCP 整合  
✅ **AI Chat 整合**：VS Code 中的自然語言查詢能力  
✅ <strong>除錯工具</strong>：全面的故障排除與連線診斷  
✅ <strong>多伺服器設置</strong>：多個 MCP 伺服器實例的配置  
✅ <strong>自訂擴充功能</strong>：附帶零售專用功能的強化 VS Code 體驗  
...
### VS Code 開發
- [VS Code 擴充 API](https://code.visualstudio.com/api) - 官方擴充開發指南
- [VS Code MCP 文件](https://code.visualstudio.com/docs/copilot/copilot-extensibility-overview) - MCP 整合文檔
...
### 開發工具
- [VS Code 中的 Python](https://code.visualstudio.com/docs/python/python-tutorial) - Python 開發環境設定
...
<strong>上一節</strong>: [實驗室 08：測試與除錯](../08-Testing/README.md)  
<strong>下一節</strong>: [實驗室 10：部署策略](../10-Deployment/README.md)
