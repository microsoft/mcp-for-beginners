...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-19T09:29:04+00:00",
...
Model Context Protocol (MCP) 已顯著成熟，增強的安全控制涵蓋傳統軟體安全及 AI 特定威脅。本文件提供截至 2025 年 12 月的完整 MCP 安全實作控制。
...
**關鍵控制：**  
- **全面授權稽核**：定期安全審查所有授權決策點  
- **失效安全預設**：授權邏輯無法明確決定時拒絕存取  
- **權限邊界**：明確區分不同權限層級與資源存取  
- **稽核日誌**：完整記錄所有授權決策以供安全監控  
- **定期存取審查**：週期性驗證用戶權限與特權分配

## 2. **令牌安全與防止令牌轉發控制**

### **禁止令牌轉發**

MCP 授權規範明確禁止令牌轉發，因其帶來嚴重安全風險：

**解決的安全風險：**  
- **控制繞過**：繞過速率限制、請求驗證及流量監控等重要安全控制  
- **責任歸屬破壞**：無法識別客戶端，破壞稽核軌跡與事件調查  
- **代理基礎資料外洩**：惡意者利用伺服器作為代理進行未授權資料存取  
...
- **短期令牌**：透過頻繁令牌輪替減少暴露時間  
- **即時發行**：僅在特定操作需要時發行令牌  
- **安全儲存**：使用硬體安全模組 (HSM) 或安全金鑰庫  
- **令牌綁定**：盡可能將令牌綁定至特定客戶端、會話或操作  
- **監控與警示**：即時偵測令牌濫用或未授權存取行為
...
- **多層安全防護**：安全架構無單點失效  
- **冗餘控制**：關鍵功能重疊安全措施  
- **失效安全機制**：系統錯誤或攻擊時採用安全預設

### **零信任實作**  
- **永不信任，持續驗證**：持續驗證所有實體與請求  
- **最小權限原則**：所有元件僅授予最低存取權限  
- **微分段**：細緻網路與存取控制

### **持續安全演進**  
- **威脅環境適應**：定期更新以應對新興威脅  
- **安全控制效能**：持續評估與改進控制措施  
- **規範遵循**：與不斷演進的 MCP 安全標準保持一致
...
> **重要**：這些安全控制反映當前 MCP 規範 (2025-06-18)。標準持續快速演進，請務必參考最新 [官方文件](https://spec.modelcontextprotocol.io/) 以確保符合最新要求。
