...
- 在處理所有工具調用的處理器中，我們嘗試將傳入請求解析為工具定義的模式：
...
接下來，我們看看如何創建工具。首先，它需要在其 *tool* 子目錄中創建，如下所示：
...
在這裡，我們添加了裝飾器 `@server.list_tools` 和實現函數 `handle_list_tools`。在後者中，我們需要生成工具列表。注意每個工具需要有名稱、描述和輸入模式。
