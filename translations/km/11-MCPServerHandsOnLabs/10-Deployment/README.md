# យុទ្ធសាស្រ្តការដាក់ចេញ

## 🎯 អ្វីដែលមន្ទីរព្យាបាលនេះគ្របដណ្តប់

មន្ទីរព្យាបាលនេះផ្ដល់ការណែនាំយ៉ាងពេញលេញអំពីការដាក់ចេញម៉ាស៊ីនបម្រើ MCP របស់អ្នកទៅបរិដ្ឋានផលិតកម្មដោយប្រើវិធីសាស្រ្ត containerization និង cloud-native ដែលទាន់សម័យ។ អ្នកនឹងរៀនរបៀបដាក់ចេញម៉ាស៊ីនបម្រើ MCP ដែលអាចបញ្ជរបាន សុវត្ថិភាព និងត្រូវបានត្រួតពិនិត្យ ដែលអាចដោះស្រាយការងារឧស្សាហកម្ម។

## ទិដ្ឋភាពទូទៅ

ការដាក់ចេញម៉ាស៊ីនបម្រើ MCP ក្នុងផលិតកម្មតម្រូវឲ្យមានការយកចិត្តទុកដាក់យ៉ាងហ្មត់ចត់លើ containerization, orchestration, សុវត្ថិភាព, មាឌ និងការត្រួតពិនិត្យ។ មន្ទីរព្យាបាលនេះគ្របដណ្តប់នូវការដាក់ចេញទៅ Azure Container Apps ជាមួយ PostgreSQL Flexible Server, អនុវត្តន៍សាខា CI/CD និងកំណត់រចនាសម្ព័ន្ធ auto-scaling សម្រាប់ផ្ទុកការងារប្រែប្រួល។

យុទ្ធសាស្រ្តការដាក់ចេញចាប់ផ្តើមពីការដាក់ចេញដុលគតែមួយសម្រាប់ការអភិវឌ្ឍទៅកាន់បរិដ្ឋានផលិតកម្មឆ្នើមដែលមានពហុតំបន់ និង auto-scaling ជាមួយការត្រួតពិនិត្យ និងមុខងារ​សុវត្ថិភាព​ដែល​ពេញលេញ។

## គោលបំណងការរៀន

នៅចុងបញ្ចប់នៃមន្ទីរព្យាបាលនេះ អ្នកនឹងអាច៖

- **ធ្វើ containerization** ម៉ាស៊ីនបម្រើ MCP ដោយប្រើ Docker ជាមួយការសាងសង់ multi-stage 
- **ដាក់ចេញ** ទៅ Azure Container Apps ជាមួយបណ្តាញសុវត្ថិភាព
- **កំណត់រចនាសម្ព័ន្ធ** PostgreSQL កម្រិតផលិតកម្មជាមួយភាពអាចរៀបចំខ្ពស់
- **អនុវត្ត** សាខា CI/CD សម្រាប់ការដាក់ចេញដោយស្វ័យប្រវត្តិ
- **ពង្រីក**កម្មវិធីដោយស្វ័យប្រវត្តិ dựa theoតម្រូវការ
- **ត្រួតពិនិត្យ** ការដាក់ចេញផលិតកម្មដោយមានការសង្កេតយ៉ាងទូលំទូលាយ
...
### Docker Compose សម្រាប់ការអភិវឌ្ឍ
...
### Docker Compose សម្រាប់ផលិតកម្ម
...
## ☁️ ការដាក់ចេញ Azure Container Apps
...
## 🚀 ការកំណត់រចនាសម្ព័ន្ធ CI/CD Pipeline
...
## 📊 ការពង្រីក និងកម្រិតកំណត់ប្រសិទ្ធភាព
...
class PerformanceMonitor:
    ...
    def __init__(self, config):
        ...
        # ការប្រមូលផ្ទាំងគុណភាព
        self.metrics_history = []
        self.request_times = []
        self.error_count = 0
        self.request_count = 0
        
        # ការត្រួតពិនិត្យមូលដ្ឋានទិន្នន័យ
        self.db_pool = None
    ...
    async def start_monitoring(self):
        ...
        # ចាប់ផ្តើមកិច្ចការប្រមូលផ្ទាំងគុណភាព
    ...
    async def _collect_metrics_loop(self):
        ...
        while True:
            try:
                ...
                if metrics.error_rate > 0.05:  # អត្រា​កំហុស ៥%
                    self.logger.warning(f"High error rate: {metrics.error_rate:.2%}")
                
                await asyncio.sleep(30)  # ប្រមូលរាល់ ៣០ វិនាទី
    ...
    async def _collect_current_metrics(self) -> PerformanceMetrics:
        ...
        request_rate = len(recent_requests) / 60.0  # សំណើរ​ក្នុងមួយវិនាទី
        
        # គណនាពេលវេលាចម្លើយមធ្យម
        ...
        return PerformanceMetrics(
            ...
            active_connections=0,  # ត្រូវអនុវត្តជាមួយការតាមដានការតភ្ជាប់
    ...
    async def _cleanup_old_metrics(self):
        ...
        while True:
            try:
                ...
                # លុបបញ្ចូលប្រវត្តិគុណភាព
                self.metrics_history = [
                    m for m in self.metrics_history
                    if m.timestamp > cutoff_time
                ]
                
                # លុបបញ្ចូលពេលវេលាសំណើរ
                self.request_times = [
                    rt for rt in self.request_times
                    if rt > cutoff_time
                ]
                
                # रीសេទក្រុមអ្នករាប់ជាថ្ងៃសំរាប់
                if datetime.utcnow().minute == 0:  # រាល់មួយម៉ោង
                    self.error_count = 0
                    self.request_count = 0
                
                await asyncio.sleep(3600)  # ប្រតិបត្តិរាល់មួយម៉ោង
...
## 🎯 ចំណុចសំខាន់ៗ ដែលត្រូវយកចិត្តទុកដាក់

បន្ទាប់ពីបញ្ចប់មន្ទីរព្យាបាលនេះ អ្នកគួរតែមាន៖

✅ **យុទ្ធសាស្រ្ត Container**: កុងតឺន័រដែលមានសុវត្ថិភាពស្រាប់សម្រាប់ផលិតកម្ម  
✅ **ការដាក់ចេញលើពពក**: Azure Container Apps ជាមួយ auto-scaling និងការត្រួតពិនិត្យ  
✅ **ការដាក់ចេញមូលដ្ឋានទិន្នន័យ**: PostgreSQL Flexible Server ជាមួយភាពអាចរៀបចំខ្ពស់  
✅ **សាខា CI/CD**: ការធ្វើតេស្ត សាងសង់ និងដាក់ចេញដោយស្វ័យប្រវត្តិ  
✅ **ការត្រួតពិនិត្យប្រសិទ្ធភាព**: ការប្រមូលមេត្រិច និងការជូនដំណឹងយ៉ាងទូលំទូលាយ  
✅ **ការកំណត់សុវត្ថិភាព**: នយោបាយសុវត្ថិភាពកម្រិតផលិតកម្ម និងការបំបែកបណ្តាញ  

## 🚀 តើអ្វីទៅជាដំណាក់កាលបន្ទាប់

បន្តជាមួយ **[Lab 11: ការត្រួតពិនិត្យ និងការសង្កេតយ៉ាងទូលំទូលាយ](../11-Monitoring/README.md)** ដើម្បី៖

- តំឡើងការត្រួតពិនិត្យទូលំទូលាយជាមួយ Application Insights  
- កំណត់ការ​រៀបចំ​កំណត់ហេតុ និងការតាមដានចែកចាយ  
- អនុវត្តការជូនដំណឹង និងប្រព័ន្ធតបតាមស្វ័យប្រវត្តិ  
- ត្រួតពិនិត្យមេត្រិចអាជីវកម្ម និង KPIs ប្រសិទ្ធភាព

## 📚 សំភារៈបន្ថែម

### បច្ចេកវិទ្យាកុងតឺន័រ
- [Docker Best Practices](https://docs.docker.com/develop/dev-best-practices/) - អនុវត្តល្អ Docker ផ្លូវការជាផ្លូវការ  
- [Azure Container Apps](https://docs.microsoft.com/en-us/azure/container-apps/) - ប្លង់ឯកសារ Azure Container Apps  
- [Kubernetes Documentation](https://kubernetes.io/docs/) - ឯកសារផ្លូវការរបស់ Kubernetes  

### CI/CD និង DevOps
- [GitHub Actions](https://docs.github.com/en/actions) - ឯកសារអំពី GitHub Actions  
- [Azure DevOps](https://docs.microsoft.com/en-us/azure/devops/) - សេវាកម្ម Azure DevOps  
- [Infrastructure as Code](https://docs.microsoft.com/en-us/azure/azure-resource-manager/bicep/) - ឯកសារ Azure Bicep  

### សុវត្ថិភាព និងការត្រួតពិនិត្យ
- [Azure Security Center](https://docs.microsoft.com/en-us/azure/security-center/) - ផ្ដល់អនុសាសន៍សុវត្ថិភាព Azure  
- [Container Security](https://kubernetes.io/docs/concepts/security/) - គំនិតសុវត្ថិភាព Kubernetes  
...
**មុននេះ**: [Lab 09: ការរួមបញ្ចូល VS Code](../09-VS-Code/README.md)  
**បន្ទាប់**: [Lab 11: ការត្រួតពិនិត្យ និងការសង្កេតយ៉ាងទូលំទូលាយ](../11-Monitoring/README.md)

---

<!-- CO-OP TRANSLATOR DISCLAIMER START -->
**ការបដិសេធ**៖  
ឯកសារនេះត្រូវបានបកប្រែដោយប្រើសេវាកម្មបកប្រែ AI [Co-op Translator](https://github.com/Azure/co-op-translator)។ ទោះយើងខ្ញុំមានការខិតខំដើម្បីឱ្យមានភាពត្រឹមត្រូវ ក៏សូមយកចិត្តទុកដាក់ថាការបកប្រែដោយស្វ័យប្រវត្តិនេះអាចមានកំហុស ឬភាពមិនត្រឹមត្រូវ។ ឯកសារដើមនៅក្នុងភាសាមូលដ្ឋានរបស់វាគួរត្រូវបានទទួលស្គាល់ថាជា ប្រភពដើមដែលមានសុពលភាព។ សម្រាប់ព័ត៌មានមានសារៈសំខាន់ គួរត្រូវបានបកប្រែដោយអ្នកជំនាញមនុស្សផ្ទាល់។ យើងខ្ញុំមិនទទួលខុសត្រូវចំពោះការយល់ច្រឡំ ឬការបកស្រាយខុសប្រកបដោយលទ្ធផលកើតឡើងពីការប្រើប្រាស់ការបកប្រែនេះឡើយ។
