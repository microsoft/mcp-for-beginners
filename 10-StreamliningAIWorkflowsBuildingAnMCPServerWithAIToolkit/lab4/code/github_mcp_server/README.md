...
2. In Microsoft Foundry Toolkit for Visual Studio Code Agent Builder, select an active Foundry deployment such as `gpt-5.1`, then enter: `What is the weather in Shanghai?` The server will be connected automatically.
...
| `.aitk`      | Microsoft Foundry Toolkit configuration      |
...
| ---------- | ----------- | --------------- |
| Agent Builder | Debug the MCP server in the Agent Builder via Microsoft Foundry Toolkit for Visual Studio Code. | 1. Open VS Code Debug panel. Select `Debug in Agent Builder` and press `F5` to start debugging the MCP server.<br>2. Select an active Foundry deployment such as `gpt-5.1`, then enter `What is the weather in Shanghai?`. The server will be connected automatically.<br>3. Click `Run` to test the server with the prompt. |
...
If you have any feedback or suggestions for this template, please open an issue on the [Microsoft Foundry Toolkit for Visual Studio Code GitHub repository](https://github.com/microsoft/vscode-ai-toolkit/issues)
